...
export default function OrderInfo({ orderNo }) {
  // [ 페이지 설정 초기화 ]
  const [isData, setIsData] = useState(false);
  // QR-code 생성할 암호처리 예약시리얼(예약번호)
  const [isQrOrderNum, setIsQrOrderNum] = useState(orderNo);
  /**
   * 예약관리 상태 state
   */
    ...
    const getDataFromServer = async () => {
      ...
      try {
        const response = await axios
          ...
          .then((res) => {
            if (res.status === 200) {
              console.log("[LOG_SW][order info Result33333] ", res.data);
    ...
    <>
      ...
      {isOrderCancel === true && (
        <div id="expire_message">
          {isCancelOrderNum === true && <p>예약번호 : {orderNumNormal.toString()}</p>}
          <p>{resultMessage}</p>
        </div>
      )}
